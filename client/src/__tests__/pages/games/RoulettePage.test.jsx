...
vi.mock('@/games/roulette/RouletteGame', () => ({
  default: () => <div data-testid="roulette-game">Roulette Game</div>,
}));
...
const renderWithProviders = (ui, { route = '/games/roulette' } = {}) =>
  render(
    <AuthProvider>
      <ToastProvider>
...
describe('RoulettePage', () => {
  it('renders without crashing inside providers and MemoryRouter', () => {
    renderWithProviders(<RoulettePage />);
    expect(screen.getByTestId('main-layout')).toBeInTheDocument();
    expect(screen.getByTestId('roulette-game')).toBeInTheDocument();
  });
  ...
  it('shows Suspense fallback while the lazy page is loading', () => {
    const LazyRoulette = lazy(() => new Promise(() => {}));
    render(
      <MemoryRouter>
        <Suspense fallback={<div data-testid="suspense-fallback">Loading Roulette...</div>}>
          <LazyRoulette />
        </Suspense>
      </MemoryRouter>
    );
    expect(screen.getByTestId('suspense-fallback')).toBeInTheDocument();
    expect(screen.getByText(/Loading Roulette/i)).toBeInTheDocument();
  });
  ...
  it('redirects to /login when AuthGuard sees no authenticated user', async () => {
    ...
    render(
      <AuthContext.Provider value={authValue}>
        <MemoryRouter initialEntries={['/games/roulette']}>
          <Routes>
            <Route
              path="/games/roulette"
              element={
    ...
    expect(screen.queryByTestId('roulette-game')).not.toBeInTheDocument();
  });
});
