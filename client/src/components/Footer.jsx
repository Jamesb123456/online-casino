...
const Footer = () => {
  ...
  return (
    <footer className="bg-bg-elevated relative z-10 border-t border-gray-800 mt-8">
          ...
          <div className="col-span-1">
            <div className="flex items-center mb-4">
              <div className="text-2xl font-bold">
                <span className="bg-gradient-primary text-transparent bg-clip-text">Virtual</span>
                <span className="bg-gradient-accent text-transparent bg-clip-text"> Casino</span>
              </div>
            </div>
            <p className="text-gray-400 mb-4">Experience the thrill of casino games in a safe, virtual environment.</p>
...
const FooterLink = ({ to, label }) => {
  return (
    <li>
      <Link to={to} className="text-gray-400 hover:text-accent transition-colors">
        {label}
      </Link>
    </li>
  );
...
const SocialButton = ({ icon }) => {
      ...
      case 'facebook':
        return (
          <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
            <path d="M9 8H6v4h3v12h5V12h3.6l.4-4h-4V6.3c0-1 .3-1.3 1.5-1.3H18V0h-3.8C10.6 0 9 1.6 9 4.6V8z"></path>
          </svg>
        );
      case 'twitter':
        return (
  ...
  return (
    <a href={`https://${icon}.com`} target="_blank" rel="noopener noreferrer" 
      className="bg-bg-card hover:bg-accent hover:text-white p-2 rounded-full transition-colors">
      {getIcon(icon)}
    </a>
  );
};
